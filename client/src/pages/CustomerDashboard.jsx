...
function CustomerDashboard() {
  ...
  const replyToOrder = async (orderId) => {
    try {
      const replyText = replyInputs[orderId] || "";
      ...
      const res = await fetch(`http://localhost:5000/api/orders/${orderId}/reply`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      if (!res.ok) {
        setMessage(data.message || "Unable to send reply.");
        return;
      }
      ...
      console.log(error);
      setMessage("Something went wrong while sending the reply.");
    }
  };
            ...
            return (
              <div key={product.id} className="rounded-xl border bg-white p-5 shadow-sm">
                ...
                <p className="mt-2 text-sm text-gray-600">
                  {product.description || "Standard packaging product"}
                </p>
                {product.name === "Custom Size Box" && (
                  <p className="mt-2 text-sm text-blue-700">
                    Use the custom box order form for custom size requirement.
                  </p>
                )}
                {product.name === "Custom Design Box" && (
                  <p className="mt-2 text-sm text-blue-700">
                    Use the custom box order form for custom design requirement.
                  </p>
                )}
              ...
              orders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td className="border p-2">
                    {order.admin_comment ? (
                      <div>
                        <p className="text-sm text-red-700">{order.admin_comment}</p>
                        <textarea
                          className="mt-2 w-full rounded border p-2 text-sm"
                          rows="2"
                          placeholder="Reply to admin"
                          value={replyInputs[order.id] ?? order.customer_reply ?? ""}
                          onChange={(e) =>
                            setReplyInputs((prev) => ({
                              ...prev,
